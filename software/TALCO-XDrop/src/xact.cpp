...
#define MARKER int((175*TILESIZE)/100)
#define DEBUG false

// void Xact::printGpuProperties () {
// }

// Xact::DeviceArrays::DeviceArrays (size_t t_tracebackArrayLength) {
//     tracebackArrayLength = t_tracebackArrayLength;
//     d_tracebackArray = (int8_t*) std::malloc(tracebackArrayLength*sizeof(int8_t));
// }

// void Xact::DeviceArrays::clearDeviceArrays () {
//     std::free(d_tracebackArray);
// }

// char* Xact::transferSequenceToDevice (char* h_sequence, size_t sequenceLength) {
//     char* d_sequence;
//     // Allocate memory
//     d_sequence = (char*) std::malloc(sizeof(char)*sequenceLength);
//     // Transfer array 
//     memcpy(d_sequence, h_sequence, sizeof(char)*sequenceLength);
//     return d_sequence;
// }

// void Xact::clearSequenceOnDevice (char* d_sequence) {
//     std::free(d_sequence);
// }
    ...
    size_t num_alignments
        ...
        for (size_t n = 0; n < num_alignments; n++) {
            ...
            while (!last_tile) {
                ...
                Xact::Tile(reference[n], query[n], params, reference_idx, query_idx, tile_aln, state, last_tile);
            ...
            score = Score(params, aln, reference[n], query[n]);
...
int32_t Xact::Reduction_tree(const int32_t *C, const int32_t start, const int32_t length){
...
void Xact::Traceback(
    ...
    while (ftr >= 0) {
        ...
        aln.push_back(state);
        if (DEBUG) std::cout << " End Addr: " << addr << std::endl;
    }
}
    ...
    bool &last_tile 
        ...
        for (int32_t k = 0; k < reference_length + query_length - 1; k++){
            ...
            if ((converged) && (max_score > conv_score)){
                conv_logic = true;
                std::cout << "Convergence logic found: ";
                break;
            }
        }
        ...
        Traceback(ftr_length, ftr_lower_limit, tb_start_addr, tb_start_ftr, (tb_state%3), query_idx, tb, aln);
        state = tb_state%3;
        if (DEBUG) {
